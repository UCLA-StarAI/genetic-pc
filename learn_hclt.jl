...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    isdir(args["dir"]) || mkpath(args["dir"])
    if !isnothing(args["k_fold"])
        filename = filename * "_k=$(args["k_fold"])"
    end
    filename = filename * "_z=$(args["latents"])_pse=$(args["pseudocount"])" * 
    ...
    @time pc, bpc, num_var = train_hclt(;kwargs...)
    if args["save_circuit"]
        write(filename, pc)
    end

    # generate 5000 samples
    df = generate_artificial_genomes(bpc, 5000, num_var)
    CSV.write("$(args["dir"])/hclt.samples", df; header=false, delim=" ")
end
